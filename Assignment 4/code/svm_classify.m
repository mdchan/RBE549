...
% extracts unique categories (15)
categories = unique(train_labels);
num_categories = length(categories);
scores = [];
for i=1:num_categories
%     need to match read categories to training labels
matchInd = strcmp(categories{i},train_labels);

% match the image feature to the binary matrix of matches output from
% strcmp
mdl =fitclinear(train_image_feats,matchInd');

% outputs a struct, with Beta and Bias fields
% use these in the equation W*X + B to calculate confidence
confidence = mdl.Beta'*test_image_feats'+mdl.Bias;

% append confidence of the features to the scores
